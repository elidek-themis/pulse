...
def delete_chat_session(session_id):
    """Delete a chat session and update current/active session if needed."""
    if session_id in st.session_state.sessions:
        del st.session_state.sessions[session_id]

    # If deleting the active session, switch to the newest remaining one
    if st.session_state.active_session == session_id:
        if st.session_state.sessions:
            latest_session = sorted(st.session_state.sessions.keys(), reverse=True)[0]
            switch_to_chat_session(latest_session)
        else:
...
def get_llm_response(messages, model, reason, content_ph=None, reasoning_ph=None):
    ...
    params = {"model": model, "messages": messages, "stream": True, "temperature": 0.0}
    if reason:
        params["extra_body"] = {"chat_template_kwargs": {"enable_thinking": True, "reasoning_effort": "high"}}
